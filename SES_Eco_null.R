...
df$lat_cat2 <- cut(df$abs_lat, 6)

colour_breaks <- c(-50, 0, 5500)
p7 <- ggplot(data = df, aes(x = PDses, y = MPFDses, col = abs_lat)) +
  geom_point( alpha = 0.8, size = .3) +
  theme_classic() +
  geom_smooth(method = 'lm', col = 'darkgrey',  lwd = .5) +
  scale_colour_gradientn( colours = colours, name = 'Abs. Latitude'
  ) +
  geom_rug(alpha = .2) +
  xlab(bquote(PD[SES])) +
  ylab(bquote(MPFD[SES])) 
p7
ggsave(plot = p7, filename = 'SESscores_Lat.pdf', device = 'pdf', dpi = 70, height = 5, width = 7)

pp7 <- p7 + facet_wrap(~lat_cat2, labeller = labeller(lat_cat = labels), nrow = 2)
pp7
ggsave(plot = pp7, filename = 'SESscores_Lat_split.pdf', device = 'pdf', dpi = 70)#, height = 5, width = 7)
...
df_standardise <- scale(df_standardise)
...
load('~/Dropbox/AVONET/Eco_NULL/distance_matrix.Rdata')
...
#covariances
############################
#run the covariances script#
############################
source('~/Dropbox/AVONET/covariances.R')

save(covariances, file = 'covariances_ses.Rdata')
load('~/Dropbox/AVONET/D_NULL/covariances_ses.Rdata')
